<script>
    ...
    export let user_id;
    let appointment_status = 'Approved';
    ...
    try {
        ...
        const response = await fetch('http://localhost:9000/booking/editAppointment', {
        method: 'PATCH',
        headers: {
...
<div class="flex justify-between items-center w-[100%] h-[10vh] border border-main px-4 rounded-lg shadow-lg">
    ...
    <button on:click={handleStart} class="text-sm py-2 px-6 rounded-lg text-black border border-main hover:bg-accent hover:text-main  ">Approve</button>
</div>